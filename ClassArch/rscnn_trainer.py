...
class RSCNNTrainer:
    ...
    def fit(self, dl_train: DataLoader, dl_test: DataLoader,
        ...
        for epoch in range(num_epochs):
            ...
            epoch_train_res = self.train_epoch(dl_train, verbose=verbose, epoch=epoch, **kw)
            ...
            if epoch % 20 == 0:
            	torch.save(self.model, f'/home/mun/workspace/rscnn_ts/ClassArch/ClassArch/epoch_save_model/{self.model.__class__.__name__}_ckpt_'+str(epoch)+'.pt')
    ...
    def train_batch(self, index, batch_data, epoch) -> BatchResult:
        """
        ...
        if self.bnm_scheduler is not None:
           self.bnm_scheduler.step(epoch-1)
        X, y = batch_data
        X = X.permute(0,2,1)
        ...
        fps_idx = pointnet2_utils.furthest_point_sample(X, 1200)  # (B, npoint)
        fps_idx = fps_idx[:, np.random.choice(1200, 1024, False)]
        X = pointnet2_utils.gather_operation(X.transpose(1, 2).contiguous(), fps_idx).transpose(1, 2).contiguous()  # (B, N, 3)
            
	# augmentation
        # X.data = PointcloudScaleAndTranslate(X.data)
    ...
    def test_batch(self, index, batch_data, epoch) -> BatchResult:
        """
        ...
        with torch.no_grad():
            ...
            # fastest point sampling
            fps_idx = pointnet2_utils.furthest_point_sample(X, 1024)  # (B, npoint)
            X = pointnet2_utils.gather_operation(X.transpose(1, 2).contiguous(), fps_idx).transpose(1, 2).contiguous()
    ...
    def _foreach_batch(dl: DataLoader,
                       forward_fn: Callable[[Any], BatchResult], epoch=-1,
        ...
        with tqdm.tqdm(desc=pbar_name, total=num_batches,
            ...
            for batch_idx in range(num_batches):
                counter += 1
                data = next(dl_iter)
                batch_res = forward_fn(batch_idx, data, epoch)
